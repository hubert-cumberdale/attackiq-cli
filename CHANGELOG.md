...
### Added
- Added stable-contract suites for the complete CLI command tree, documentation parity,
  persisted configuration, environment variables, exit behavior, and machine-readable output
  shapes.
- Added a bounded live-smoke roster for configuration, local-spec, read-only inventory, and
  fake-ID mutation dry-run checks. Live execution is capped at five records per inventory call and
  fails closed unless the effective base URL uses HTTPS with TLS verification enabled.

### Changed
- Reject non-finite persisted timeouts and TUI cache TTL values while preserving the documented
  configuration keys, defaults, and warning-only `config validate` behavior.
- Qualified Python 3.13 and expanded the exact GitHub Actions matrix to Python 3.10 through 3.13
  without changing the Python `>=3.10`, Ruff, or mypy baselines.
- Classified the prepared `v1.1.0` package metadata as Production/Stable. The candidate remains
  explicitly non-GA until the separately authorized production rollout and promotion gates pass.
