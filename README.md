...
- List integration source types with `attackiq source-types list` for a known company and connector.
- List assessment project schedules with `attackiq assessment-schedules list` using summary-only
  read-only output.
- List EDR scan schedules with `attackiq edr-scan-schedules list` using summary-only read-only
  output that omits raw target asset IDs.
...
- Contextual Scenarios-, Assessments-, and Tests-tab request previews for the existing
  new-assessment, assessment-from-template, assessment-run, assessment-default targets, new-test,
  test-scenario assignment, and test-status dry-run plans, rendered locally with
  `No request sent` status and no apply path.
...
# Build payloads (no network) and preview generic calls without sending requests
...
attackiq call v1_assessments_project_from_template_create \
  --body-file create_assessment.json \
  --dry-run
...
attackiq call v1_tests_create --body-file create_test.json --dry-run
...
- Path, query, body, and form values appear in dry-run output as supplied; do not include secrets
  in those fields when capturing or sharing a preview.
...
- The output reports wrapper version, wrapper-only status, and the expected local runtime bundle
  path used by the implemented no-container create flow.
...
- `attackiq source-types list --company-id <company-id> --connector-id <connector-id>`
- `attackiq source-types list --company-id <company-id> --connector-id <connector-id> --output-format csv --output source-types.csv`

Notes:
- `source-types list` supports required `--company-id` and `--connector-id` UUIDs plus `--page`,
  `--page-size`, `--object-fingerprint`, `--unassigned-for`, `--output-format`, `--output`,
  `--insecure`, and `--timeout`.
- JSON and CSV output use a summary record shape and do not include connector configuration.
- CSV output requires `--output`.

## Assessment Schedule commands
- `attackiq assessment-schedules list --output assessment-schedules.json`
- `attackiq assessment-schedules list --output-format csv --output assessment-schedules.csv`

Notes:
- `assessment-schedules list` supports `--output-format`, `--output`, `--insecure`, and
  `--timeout`.
- The bundled schema endpoint is unpaginated; output uses a summary record shape with project name,
  project template name, schedule version, crontab fields, timezone, and schedule-present status.
- The command does not set, clear, enable, disable, create, update, or delete schedules.
- CSV output requires `--output`.

## EDR Scan Schedule commands
- `attackiq edr-scan-schedules list --enabled true --output edr-scan-schedules.json`
- `attackiq edr-scan-schedules list --schedule-type WEEKLY --output-format csv --output edr-scan-schedules.csv`

Notes:
- `edr-scan-schedules list` supports `--page`, `--page-size`, `--data-source`, `--enabled`,
  `--schedule-type`, `--targeted`, `--output-format`, `--output`, `--insecure`, and `--timeout`.
- JSON and CSV output use a summary record shape that omits raw `target_asset_ids` and derives
  target scope through `targeted` and `target_asset_count`.
- The command does not retrieve schedule detail, fetch run history, create, update, delete, enable,
  disable, or apply schedules.
- CSV output requires `--output`.

## Backup commands
- `attackiq backup configs --output-dir /tmp/aiq-config-backup-20260521T120000Z --tenant-alias tenant-a`
- `attackiq backup configs --output-dir /tmp/aiq-config-backup-20260521T120000Z --include integrations,source-types --max-pages 5`
- `attackiq backup configs --include observable-field-mappings --endpoint-catalog docs/CONFIGURATION_BACKUP_ENDPOINT_CATALOG.example.json --output-dir /tmp/aiq-observable-field-mappings-backup`
...
- Optional endpoint-catalog domains such as `observable-field-mappings` require both `--include`
  and `--endpoint-catalog`; they are not default domains and do not have first-class CLI wrappers.
...
- On the Scenarios tab, `Ctrl+K` -> `Preview new assessment request` accepts or prefills a
  scenario UUID, accepts additional comma-separated scenario UUIDs, requires an assessment name,
  and renders the local request body without sending or exporting it.
- On the Assessments tab, `Ctrl+K` -> `Preview assessment run request` opens a local call-plan
  preview. It accepts or prefills an assessment UUID, sends no request, and does not export the
  preview.
- On the Assessments tab, `Ctrl+K` -> `Preview assessment from template request` requires a
  template UUID and assessment name, accepts an optional blueprint UUID, and renders the local
  request body without sending or exporting it.
- On the Assessments tab, `Ctrl+K` -> `Preview new test request` accepts or prefills an assessment
  UUID, requires a test name, and renders the local request body without sending or exporting it.
- On the Assessments tab, `Ctrl+K` -> `Preview assessment default targets request` accepts or
  prefills an assessment UUID and requires at least one comma-separated asset or asset-group UUID
  list. It renders locally without sending or exporting the request.
- On the Tests tab, `Ctrl+K` -> `Preview test status request` opens a local call-plan preview. It
  accepts or prefills a test UUID, sends no request, and does not export the preview.
- On the Tests tab, `Ctrl+K` -> `Preview test scenario assignment request` accepts or prefills a
  test UUID plus comma-separated scenario UUIDs, then renders a bounded local request body without
  sending or exporting it.
...
- CI and local release gates run `python3 scripts/check_secret_scan.py` with the explicit
  `security/secret-scan-allowlist.json` allowlist. Findings report only path, line, and rule label.

## Dependency Governance
- Keep `pyproject.toml` ranges conservative and maintain `constraints.txt` for releases.
- Recommended tooling:
  ...
  - `python3 scripts/check_secret_scan.py` checks source files for likely committed secrets using
    the reviewed allowlist.
  - `docs/RELEASE_PREP_EVIDENCE_CHECKLIST.md` standardizes pre-tag evidence for dependency
    constraints, public safety, secret scan, public mirror dry-run, `pip-audit`, constraints audit,
  ...
  - `python3 scripts/verify_enterprise_package.py <dir> --require-constraints` verifies the package manifest,
    checksums, checked install constraints, dependency integrity record, SBOM, provenance, safe
    artifact names, and wheel public-safety status before or after Artifactory.
  - `python3 scripts/build_artifactory_promotion_evidence.py <dir>` creates credential-free
    Artifactory promotion evidence with upload filenames, checksums, target paths, and post-upload
    verification steps.
  - `python3 scripts/build_signing_attestation_evidence.py <dir>` creates credential-free signing
    and attestation evidence with subject digests, expected signature files, expected attestation
    files, predicate requirements, and standardized external evidence field groups.
  - `python3 scripts/verify_enterprise_evidence.py <dir> --require-artifactory --require-signing`
    cross-checks generated Artifactory and signing evidence against local package artifacts before
    release records are accepted.
  - `docs/POST_DOWNLOAD_PACKAGE_EVIDENCE_CHECKLIST.md` standardizes final cross-checks for the
    downloaded wheel, constraints, SBOM, dependency integrity, provenance, signatures,
    attestations, trust-root verification, and install smoke evidence.
  - `pip-audit` against the installed release environment, plus
    `pip-audit -r constraints.txt --no-deps` when auditing the pinned constraints record directly.
