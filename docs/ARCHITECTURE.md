...
- TUI entrypoint and helpers: `src/attackiq_cli/tui.py`,
  `src/attackiq_cli/tui_app.py`, `src/attackiq_cli/tui_assessments.py`,
  `src/attackiq_cli/tui_assets.py`, `src/attackiq_cli/tui_provider.py`,
  `src/attackiq_cli/tui_provider_state.py`,
  `src/attackiq_cli/tui_display.py`, `src/attackiq_cli/tui_domains.py`,
  `src/attackiq_cli/tui_exports.py`,
  `src/attackiq_cli/tui_filters.py`, `src/attackiq_cli/tui_record_lists.py`,
  `src/attackiq_cli/tui_record_text.py`, `src/attackiq_cli/tui_results.py`,
  `src/attackiq_cli/tui_tasks.py`, `src/attackiq_cli/tui_scenarios.py`,
  `src/attackiq_cli/tui_settings.py`,
  `src/attackiq_cli/tui_styles.py`, `src/attackiq_cli/tui_tests.py`, and
  `src/attackiq_cli/tui_widgets.py`
...
- `src/attackiq_cli/tui.py`: TUI launch orchestration and compatibility imports for extracted TUI
  symbols.
- `src/attackiq_cli/tui_app.py`: Textual app shell, tab orchestration, command-palette dispatch,
  cache/status actions, paging/export routing, help, and focus controls.
- `src/attackiq_cli/tui_provider.py`: read-only service fetching, bounded in-session caches,
  cache statistics and clearing, workspace resolution, and compatibility imports for runtime state
  symbols.
- `src/attackiq_cli/tui_provider_state.py`: TUI runtime state model and pure auth, base URL, spec
  cache, environment-label, and workspace-display derivation.
- `src/attackiq_cli/tui_assessments.py`: Assessments tab state, async loading, rendering,
  filtering, paging, detail, export actions, and view-state restoration.
- `src/attackiq_cli/tui_assets.py`: Assets tab state, async loading, rendering, filtering, paging,
  detail, export actions, and view-state restoration.
- `src/attackiq_cli/tui_display.py`: pure TUI shortcut text, command-palette matching/group hints,
...
- `src/attackiq_cli/tui_exports.py`: shared TUI timestamped export path construction and JSON/CSV
  file writing helpers used by list tabs.
- `src/attackiq_cli/tui_filters.py`: pure TUI structured-filter parsing, typed filter coercion,
  and sort-field resolution helpers shared by tabs.
- `src/attackiq_cli/tui_record_lists.py`: pure TUI result grouping, list filtering, and
  scenario/assessment/test/asset/settings/results sort helpers.
- `src/attackiq_cli/tui_record_text.py`: pure TUI record ID extraction, fallback labels, and
  scenario/assessment/test/asset detail text builders.
- `src/attackiq_cli/tui_results.py`: Results tab models, state, async loading, view-mode grouping,
  rendering, filtering, paging, detail, export actions, and view-state restoration.
- `src/attackiq_cli/tui_scenarios.py`: Scenarios tab state, async loading, rendering, filtering,
  paging, detail, export actions, and view-state restoration.
- `src/attackiq_cli/tui_settings.py`: Settings tab state, rendering, filtering, export actions,
  and record/detail builders for runtime/config/cache diagnostics.
- `src/attackiq_cli/tui_tasks.py`: TUI task lifecycle and blocking executor handoff helpers for
  cancellation, task replacement, debounced filter reload scheduling, and tab background work.
- `src/attackiq_cli/tui_tests.py`: Tests tab state, async loading, rendering, filtering, paging,
  detail, export actions, and view-state restoration.
- `src/attackiq_cli/tui_styles.py`: shared Textual stylesheet for the TUI app shell, overlays,
  panes, filters, footer, and results status display.
- `src/attackiq_cli/tui_widgets.py`: shared TUI header/banner/filter/list/detail/status widgets
  and placeholder workflow tab shell.
...
- `src/attackiq_cli/tui_preview.py`: local new-assessment, assessment-from-template,
  assessment-default targets, assessment-run, new-test, test-scenario assignment, and test-status
  request-preview validation, shared rendering, and Textual modal controls over the shared dry-run
  call-plan and redaction adapter.
...
- `src/attackiq_cli/scenario_wizard.py`: Scenario Wizard image-tar prepare planning/apply
  orchestration and compatibility exports for focused create, image, package, process,
  bundle-runtime, and validation modules.
- `src/attackiq_cli/scenario_wizard_create.py`: Scenario Wizard create dry-run planning, isolated
  fixture-backed apply execution, temporary configuration transport, runtime dependency setup, and
  generated-file result collection.
- `src/attackiq_cli/scenario_wizard_image.py`: Scenario Wizard image-tar runtime inspection,
  bounded layer spooling, Docker whiteout/index handling, selected-file materialization, safe path
  normalization, sensitive-file exclusion, and requirements credential filtering.
...
- `src/attackiq_cli/scenario_wizard_process.py`: allowlisted local subprocess environment
  ...
  workflows.
- `src/attackiq_cli/scenario_wizard_runtime.py`: Scenario Wizard cache resolution, wrapper ZIP
  inspection with sensitive-file suppression, runtime-bundle validation summaries, bundle-copy
  dry-run planning, and bundle-copy apply behavior.
- `src/attackiq_cli/joiner/det_pipeline.py`: deterministic DET stages A-E, artifact writing,
  dry-run/apply report orchestration, and compatibility exports for apply executors.
- `src/attackiq_cli/joiner/det_pipeline_apply.py`: GitLab retry/client behavior plus apply-gated
  GitLab issue updates and AttackIQ assessment creation.
- `src/attackiq_cli/joiner/*`: remaining deterministic join helpers for AttackIQ exports and
  GitLab issues CSVs.
