...
python3 scripts/build_enterprise_package.py \
  --source-ref v0.1.23 \
  --output-dir /tmp/attackiq-cli-enterprise-package-v0.1.23
python3 scripts/verify_enterprise_package.py /tmp/attackiq-cli-enterprise-package-v0.1.23 --require-constraints
```
...
python3 scripts/build_artifactory_promotion_evidence.py \
  /tmp/attackiq-cli-enterprise-package-v0.1.23 \
  --artifactory-url https://artifactory.example.com/artifactory \
  --repository-path api/pypi/attackiq-cli-local \
  --output /tmp/attackiq-cli-enterprise-package-v0.1.23/ARTIFACTORY_PROMOTION_EVIDENCE.json
```
...
python3 scripts/build_signing_attestation_evidence.py \
  ...
  --output /tmp/attackiq-cli-enterprise-package-v0.1.23/SIGNING_ATTESTATION_EVIDENCE.json
```
...
```bash
python3 scripts/verify_enterprise_package.py /tmp/attackiq-cli-artifactory-download-v0.1.23 --require-constraints
```
