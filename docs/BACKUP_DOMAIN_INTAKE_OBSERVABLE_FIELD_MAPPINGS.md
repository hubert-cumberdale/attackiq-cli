...
When this optional endpoint-catalog domain is requested, its generated artifact remains
...
Generated backup directories for this optional domain must stay outside git, use the existing
backup output permission model, and be retained only for the approved evidence window. Public docs
may record aggregate status such as command success, record count, redaction status, and skipped
...
Do not add this domain to default coverage or create a first-class wrapper without a separate
review. Keep operator examples explicit about endpoint-catalog use.
...
Use these commands when changing this optional endpoint-catalog domain:
