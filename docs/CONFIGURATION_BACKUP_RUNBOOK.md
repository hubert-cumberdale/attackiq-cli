...
Optional endpoint-catalog coverage can be requested explicitly after catalog review:
...
This keeps `observable-field-mappings` endpoint-catalog only. It is not part of default coverage
and does not have a first-class CLI wrapper.
...
- `detection-rules`: fetched through the read-only
  ...
  The separate wrapper review keeps this backup-only until a safe summary projection and wrapper
  boundary are specified, so backup redaction stays enabled.
- `observable-field-mappings`: optional endpoint-catalog coverage only, requested with
  `--include observable-field-mappings` plus the sanitized endpoint catalog example. It remains
  classified as `needs-redaction` and is not included in default coverage.
...
Reviewed intake notes:

- [Observable field mappings](BACKUP_DOMAIN_INTAKE_OBSERVABLE_FIELD_MAPPINGS.md): optional
  `needs-redaction` endpoint-catalog backup domain with fixture-backed redaction, manifest, and
  retention guidance; not enabled in CLI defaults.
- [Detection rule wrapper review](DETECTION_RULE_WRAPPER_REVIEW.md): keeps detection/alert-rule
  candidates backup-only until a safe read-only wrapper projection and service boundary are
  specified.
