...
validated by `scripts/check_aiq_assist_mcp_contract.py`. CLI/TUI consumer code stays blocked until
the provider source is documented and the status explicitly records both consumption approval and
adapter mock-test evidence. Default live MCP checks stay blocked.
