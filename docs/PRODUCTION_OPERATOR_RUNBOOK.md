...
production-ready release line is `v0.1.19` for standard documented CLI workflows. Continue to use
...
Before upload and after download, verify the package artifact directory, including checksums,
promotion manifest, and package provenance, from a release checkout:

```bash
python3 scripts/verify_enterprise_package.py /tmp/attackiq-cli-enterprise-package-v0.1.19
python3 scripts/build_artifactory_promotion_evidence.py \
  /tmp/attackiq-cli-enterprise-package-v0.1.19 \
  --artifactory-url https://artifactory.example.com/artifactory \
  --repository-path api/pypi/attackiq-cli-local \
  --output /tmp/attackiq-cli-enterprise-package-v0.1.19/ARTIFACTORY_PROMOTION_EVIDENCE.json
```

Keep the generated Artifactory evidence in the enterprise release record if it includes internal
repository coordinates; do not commit it to this repository.

If your organization has promoted the release wheel to Artifactory or another approved enterprise
package repository, install from that package index with the `constraints.txt` from the public
release tag or an approved internal constraints record:
...
Do not commit package-index credentials, pip config files, `.netrc` files, or downloaded wheels.
