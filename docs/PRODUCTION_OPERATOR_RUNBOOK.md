...
production-ready release line is `v0.1.27` for standard documented CLI workflows. Continue to use
...
`constraints.txt`, checksums, promotion manifest, dependency integrity, SBOM, and package
provenance, from a release checkout:

```bash
release_package_dir="<package-dir-outside-git>"
python3 scripts/verify_enterprise_package.py "$release_package_dir" --require-constraints
python3 scripts/build_artifactory_promotion_evidence.py \
  ...
  --output "$release_package_dir/ARTIFACTORY_PROMOTION_EVIDENCE.json"
python3 scripts/build_signing_attestation_evidence.py \
  "$release_package_dir" \
  --signing-profile enterprise-release \
  --output "$release_package_dir/SIGNING_ATTESTATION_EVIDENCE.json"
...
internal repository coordinates or signing profile names; do not commit it to this repository. The
evidence verifier checks local evidence consistency and the standardized external
signing/attestation/trust-root field groups; enterprise signing trust-root verification remains
operator-owned.

When direct Artifactory access is unavailable, complete the public-safe evidence record in
`docs/NO_ARTIFACTORY_RELEASE_EVIDENCE_TEMPLATE.md` for package build, package verification,
credential-free Artifactory/signing evidence generation, enterprise evidence verification, and the
local wheelhouse install simulation. Store any filled template with private paths, operator names,
change-ticket IDs, or trust-root identifiers in the approved enterprise release system, not in git.
`docs/RELEASE_AUDIT_WRAPPER_EVALUATION.md` explains why the current release line uses this explicit
template path rather than a repository-owned executing wrapper.

After package download, complete `docs/POST_DOWNLOAD_PACKAGE_EVIDENCE_CHECKLIST.md` to cross-check
package artifacts, signatures, attestations, trust-root verification, and install smoke evidence
against one external release record.
...
Use least-privilege package credentials: scope them to the approved package repository/path, grant
only upload plus read-back verification rights for promotion operators, and keep delete,
administration, and cross-repository promotion rights out of release tokens. Signing and attestation
verification must use the enterprise-approved trust root, with the trust-root identifier and
verified subject digests recorded against the standardized external evidence fields in the
...
output. Live execution refuses page sizes above 5 and, before launching any command, fails unless
the effective base URL uses `https://` and persisted configuration has TLS verification enabled.
The plan-only `--dry-run` skips this preflight and remains offline.
...
- Read-only tenant inventory with bounded pages:
  ...
  - `attackiq source-types list --company-id <company-id> --connector-id <connector-id> --page 1 --page-size 5 --output /tmp/aiq-source-types.json`
...
- Read-only schedule inventory:
  - `attackiq assessment-schedules list --output /tmp/aiq-assessment-schedules.json`
  - `attackiq edr-scan-schedules list --page 1 --page-size 5 --output /tmp/aiq-edr-scan-schedules.json`
  - The accepted assessment schedule endpoint is unpaginated in the bundled schema; review output
    size expectations before running in very large tenants.
  - EDR scan schedule output is summary-only and omits raw `target_asset_ids`; retrieve, runs,
    create, update, and delete behavior is out of scope for the wrapper.
...
Optional endpoint-catalog coverage:
...
`observable-field-mappings` is endpoint-catalog only, remains classified as `needs-redaction`, and
is not part of default coverage.
