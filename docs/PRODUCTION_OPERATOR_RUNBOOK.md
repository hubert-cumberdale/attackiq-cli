...
production-ready release line is `v0.1.24` for standard documented CLI workflows. Continue to use
...
```bash
python3 scripts/verify_enterprise_package.py /tmp/attackiq-cli-enterprise-package-v0.1.24 --require-constraints
python3 scripts/build_artifactory_promotion_evidence.py \
  /tmp/attackiq-cli-enterprise-package-v0.1.24 \
  --artifactory-url https://artifactory.example.com/artifactory \
  --repository-path api/pypi/attackiq-cli-local \
  --output /tmp/attackiq-cli-enterprise-package-v0.1.24/ARTIFACTORY_PROMOTION_EVIDENCE.json
python3 scripts/build_signing_attestation_evidence.py \
  /tmp/attackiq-cli-enterprise-package-v0.1.24 \
  --signing-profile enterprise-release \
  --output /tmp/attackiq-cli-enterprise-package-v0.1.24/SIGNING_ATTESTATION_EVIDENCE.json
