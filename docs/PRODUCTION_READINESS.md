...
Current release status: `v0.1.25` remains the latest published/tagged production release.
`v0.1.26` release-candidate metadata is prepared for standard documented CLI work and
credential-free enterprise package promotion as of 2026-06-25 after read-only wrapper expansion,
TUI dry-run preview planning/tests, release-prep evidence checklist additions, post-download
package evidence guidance, AIQ Assist MCP contract/fixture expansion, continued architecture
decomposition, completed CLI command-family extraction, documentation rebaselining, release
governance checks, and the full local quality gate. Tag creation, strict public mirror validation,
public GitHub release creation, and enterprise package promotion evidence remain pending explicit
release approval.
Continue to require
