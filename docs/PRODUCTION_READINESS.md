...
Current release status: `v0.1.24` is production-ready for standard documented CLI work and
credential-free enterprise package promotion as of 2026-05-27 after public-safety cleanup, public
mirror validation, package promotion artifact validation, checked install constraints, fail-closed
constraints verification, offline package verification, package provenance, Artifactory promotion
evidence generation, signing/attestation evidence generation, enterprise evidence verification,
local quality gates,
Node 24-compatible GitHub Actions, release governance checks, and package guard updates.
Continue to require
...
| Enterprise package artifacts | `python3 scripts/build_enterprise_package.py --source-ref vX.Y.Z --output-dir <dir>` and `python3 scripts/verify_enterprise_package.py <dir> --require-constraints` pass for the public tag and produced package directory, including declared install constraints and package provenance. |
