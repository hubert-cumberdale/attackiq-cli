...
Current release status: `v0.1.20` is production-ready for standard documented CLI work and
credential-free enterprise package promotion as of 2026-05-26 after public-safety cleanup, public
mirror validation, package promotion artifact validation, offline package verification, package provenance,
Artifactory promotion evidence generation, signing/attestation evidence generation, local quality
...
| Enterprise package artifacts | `python3 scripts/build_enterprise_package.py --source-ref vX.Y.Z --output-dir <dir>` and `python3 scripts/verify_enterprise_package.py <dir>` pass for the public tag and produced package directory, including declared package provenance. |
