...
Current release status: `v0.1.15` is production-ready for standard documented CLI work and
credential-free enterprise package promotion as of 2026-05-24 after public-safety cleanup, public
mirror validation, package promotion artifact validation, local quality gates, release governance
checks, and package guard updates. Continue to require workflow-specific approval for destructive,
high-volume, custom-scenario, restore/apply, or tenant-data-heavy activity.
...
| Enterprise package artifacts | `python3 scripts/build_enterprise_package.py --source-ref vX.Y.Z --output-dir <dir>` passes for the public tag and produces a wheel, `SHA256SUMS`, and `ENTERPRISE_PROMOTION_MANIFEST.json`. |
...
2. Audit: run dependency audit, security/redaction review, public-safety, public-mirror,
   enterprise-package, and docs integrity checks.
