...
2. Complete publication and verification of the explicitly non-GA `v1.1.0` stable candidate,
   then stop before the separately authorized Gate 5 production rollout.
3. Keep `v0.1.27` as the current production-ready Beta and keep candidate and release evidence
   aligned while preserving generated artifacts outside git.
4. Keep the public release line and one-commit public mirror stable.
5. Keep GitHub Actions and `scripts/quality_gate.py` aligned so release checks have one practical
   source of truth.
6. Enforce the completed #55 architecture boundaries through the local and CI 800-line module
   check, and require a cohesive ownership need before further extraction.
7. Maintain redacted configuration backup coverage and endpoint-catalog validation.
8. Keep #60 in watch mode after the 2026-07-17 gate selected no new wrapper; reopen selection only
   for a named workflow with a safe projection and retention/redaction evidence.
9. Mature AIQ Assist MCP contracts before adding CLI or TUI consumption.
10. Keep the implemented new-assessment, assessment-from-template, assessment-default targets,
...
- Release closeout: issue #76 tracked version/changelog/metadata release prep, full gate evidence,
  and tag-time checks for `v0.1.26`. The `v0.1.27` source and public releases, tag-time CI, public
  snapshot, and enterprise verification are recorded in `docs/STATE.md` and
  `docs/MAINTENANCE.md`. Current `master` is past the `v0.1.27` tag and should use normal
  release-prep evidence before any future tag.
...
## General Availability Graduation Track

`v1.1.0` is the prepared first governed stable candidate for enterprise operators. `v0.1.27`
remains the current production-ready Beta until every gate below passes. Candidate preparation and
Gate 4 publication do not authorize tenant activity or declare GA in `docs/STATE.md`; both GitHub
release records remain prereleases until a later promotion is separately authorized.
...
2. Inventory the stable CLI and configuration contract, then add contract-test tasks for every
   ...
   machine-readable output shape. The planning baseline and task-status backlog are tracked in
   `docs/GA_STABLE_CONTRACT.md`; the command-tree fixture, documentation parity classifications,
...
The release record must assign a release owner, production operator, security reviewer, and
rollback owner before Gate 5. Role assignments and escalation details may be retained in the
approved enterprise system; public documentation must record accountability without exposing
private identities.

GA rollout evidence is limited to the version, public commit, UTC checkpoints, command categories,
aggregate results, incidents, and rollback status. Credentials, tenant URLs, tenant or operator
identifiers, raw responses, and local paths remain outside git. Issue #60 (wrapper expansion) and
issue #61 (supply-chain policy) remain non-blocking watch epics unless a rollout finding creates a
specific GA blocker. AIQ Assist MCP consumption remains disabled, and apply-mode workflows require
separate future approval after GA.
...
- Fixed deterministic review worktree status reporting so empty `git status --short` output
  ...
  (2026-07-16).
- Published and promoted `v0.1.27` as the current production release after strict public mirror,
  tag-time CI, enterprise package, combined evidence verification, and offline wheelhouse install
  evidence were recorded (2026-07-20).
- Published and promoted `v0.1.26` as the current production release at that time after strict
  public mirror, tag-time CI, enterprise package, combined evidence verification, and offline
  wheelhouse install evidence were recorded (2026-06-25).
