...
This document describes the implemented local `create` sequence for AttackIQ Scenario Wizard
without running the Docker container at scenario creation time.
...
Current implemented support includes read-only inspection, runtime bundle validation, and
...
The create/package steps do not require a running container when a valid local runtime bundle
exists.

## Implemented Architecture

The workflow uses a versioned local runtime bundle.
