...
- Current production-ready release: `v0.1.19`.
- Release note: `v0.1.19` adds credential-free Artifactory promotion evidence generation for
  verified enterprise package directories, including upload file metadata, target coordinates,
  post-download verification steps, and operator-owned signing/attestation boundaries.
- Previous release: `v0.1.18` updated GitHub Actions to Node 24-compatible official action majors
  and removed the Node 20 deprecation warning from CI.
...
- Build validated enterprise package promotion artifacts from public release tags without storing
  registry credentials, generate offline package provenance/dependency inventory, verify
  generated or downloaded package artifact directories before promotion or install, and create
  credential-free Artifactory promotion evidence for operator-owned uploads.
...
tagged ref, such as `python3 scripts/check_public_mirror.py --ref v0.1.19`.
...
wheel into an operator-selected output directory outside the repo, writes `SHA256SUMS`, records
`ENTERPRISE_PROMOTION_MANIFEST.json`, and writes `ENTERPRISE_PACKAGE_PROVENANCE.json`.
`scripts/verify_enterprise_package.py` independently checks the package directory manifest,
checksums, safe artifact names, wheel public-safety status, and declared provenance before
Artifactory upload or after download from an enterprise package repository.
...
These scripts do not upload to Artifactory, accept registry credentials, sign artifacts, or attach
package artifacts to the public GitHub release. Operators promote the generated wheel, checksums,
manifest, provenance, and optional promotion evidence through their approved enterprise package
