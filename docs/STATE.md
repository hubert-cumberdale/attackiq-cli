...
- Release note: `v0.1.25` adds the post-#75 roadmap slices: first architecture decompositions,
  `attackiq source-types list`, enterprise SBOM/dependency-integrity artifacts, allowlisted
  source-secret scanning, and CI/local quality-gate parity.
- Release-candidate note: metadata and changelog have been prepared for `v0.1.25`, but no tag or
  GitHub release has been created in this pass. See
  `docs/REVIEW_2026-05-29_POST_75_RELEASE_READINESS.md`.
- Previous release: `v0.1.24` adds offline verification for generated enterprise Artifactory and
...
- Provide first-class read-only wrappers for tags, scenarios, assets, asset groups, blueprints,
  integrations, source types, templates, results, validation results, assessments, and tests.
...
tagged ref, such as `python3 scripts/check_public_mirror.py --ref v0.1.25`.
...
Post-#75 CI on `master` passed for merge commit
