...
- Current production-ready release: `v0.1.16`.
- Release note: `v0.1.16` adds offline enterprise package verification for generated or
  Artifactory-downloaded package directories, including manifest/schema checks, checksum matching,
  safe artifact names, and wheel public-safety scanning.
- Previous release: `v0.1.15` added credential-free enterprise package promotion tooling that
  builds validated wheel artifacts from public release tags without accepting Artifactory
  credentials.
...
- Build validated enterprise package promotion artifacts from public release tags without storing
  registry credentials, and verify generated or downloaded package artifact directories before
  promotion or install.
...
tagged ref, such as `python3 scripts/check_public_mirror.py --ref v0.1.16`.
...
wheel into an operator-selected output directory outside the repo, writes `SHA256SUMS`, and records
`ENTERPRISE_PROMOTION_MANIFEST.json`. `scripts/verify_enterprise_package.py` independently checks
the package directory manifest, checksums, safe artifact names, and wheel public-safety status
before Artifactory upload or after download from an enterprise package repository.

The script does not upload to Artifactory, accept registry credentials, generate an SBOM, or attach
package artifacts to the public GitHub release. Operators promote the generated wheel and checksums
through their approved enterprise package workflow outside this repository.
