...
- Lifecycle status: production-capable, mature Beta.
- Current production-ready release: `v0.1.27`.
- Prepared release candidate: `v1.1.0`.
- `v1.1.0` is the prepared first governed stable candidate for enterprise operators. Its package
  metadata is Production/Stable, but the candidate is explicitly non-GA and does not replace
  `v0.1.27` as the current production-ready Beta. Its private-source and public GitHub releases
  must be prereleases with no attached artifacts and remain prereleases until a later GA
  promotion is separately authorized and completed.
- GA graduation status: Gates 1-3 are complete. Gate 3 closed on 2026-07-21 when PR #166
  squash-merged as commit `04f999d039d26684891cea40e04e2b8df20fc46f` and post-merge CI run
  `29856520468` passed the exact Python 3.10, 3.11, 3.12, and 3.13 matrix. The qualifying fresh,
  constrained, uv-managed CPython 3.13.12 environment passed the full quality gate with 898 tests
  and both the installed-environment and direct-constraints dependency audits reported no known
  vulnerabilities.
- Gate 4 is authorized and in progress. Candidate metadata and the Production/Stable classifier
  are prepared without changing Python `>=3.10`, Ruff/mypy 3.10 baselines, dependencies, API
  behavior, or apply restrictions. The tag, releases, strict public snapshot, and enterprise
  evidence remain incomplete until recorded after immutable publication. Gate 5 production
  rollout remains separately authorized and must not begin as part of Gate 4.
- The GA milestone certifies existing read-only and fake-ID mutation dry-run workflows only. Apply
  operations, PyPI and broad public-package support, AIQ Assist MCP consumption, and new wrapper or
  TUI expansion remain outside scope and require separate future approval.
- `v0.1.27` was tagged, published, and promoted on 2026-07-20. The private source tag resolves to
  commit `6c26c502bfa16000fcb6bf8c4482b414f9bf8963`, tag-time CI run `29769671071` passed, and the
  public lightweight tag and public `main` resolve to one-commit snapshot
  `0af7861abebd0d5ef9f3443b389b54d5f30da3eb`. The exact-SHA public `main` correction passed the
  full Python 3.10-3.12 matrix in CI run `29778274916`.
- Release note: `v0.1.27` completes the seven approved read-only TUI previews and the identified
  architecture decomposition, enforces the 800-line source-module boundary, expands fail-closed
  AIQ Assist MCP contract and fixture guardrails while keeping consumption disabled, and aligns
  operator documentation with observable CLI behavior.
- Release validation evidence: dependency constraints, release governance, public safety, secret
  scan, strict public mirror, both dependency audits, the 761-test quality gate, documentation
  links, deep-dive checks, strict MkDocs, and whitespace checks passed. Enterprise package,
  checksum, SBOM, dependency-integrity, provenance, Artifactory-promotion, signing-attestation,
  combined evidence, and offline wheelhouse install verification also passed; public-safe digests
  and results are recorded in `docs/MAINTENANCE.md`.
- `v0.1.26` was the previous published/tagged release as of 2026-06-25. The source release commit is
...
- Current production-ready release evidence: source and public GitHub releases were published for
  `v0.1.27`; strict public mirror, enterprise package, SBOM, dependency-integrity, provenance,
...
- Controlled non-production operational soak: public `v0.1.27` commit
  ...
  `2026-07-20T20:06:03Z` across configuration validation, local spec discovery, five bounded
  read-only inventory calls, and four fake-ID dry-run call plans. No apply-mode operation ran;
  aggregate evidence is recorded in `docs/MAINTENANCE.md`.
...
The `v0.1.27` tag-time CI evidence passed for source commit
`6c26c502bfa16000fcb6bf8c4482b414f9bf8963` in run `29769671071`, including Python 3.10, 3.11,
