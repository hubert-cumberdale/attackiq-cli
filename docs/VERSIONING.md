...
- The current production-ready release is the version declared in `docs/STATE.md`, and it must
  match `pyproject.toml`, `src/attackiq_cli/__init__.py`, `attackiq --version`, `CHANGELOG.md`,
  and the release tag for that release.
...
- `scripts/check_release_governance.py` enforces that the documented production-ready release line
  follows package metadata and that the historical `v1.0.0` exception remains documented.
