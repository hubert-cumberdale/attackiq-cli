...
def build_promotion_manifest(
    *,
    ...
    dependency_integrity_sha256: str | None = None,
    provenance_filename: str = PROVENANCE_NAME,
    sbom_filename: str = SBOM_NAME,
    dependency_integrity_filename: str = DEPENDENCY_INTEGRITY_NAME,
    constraints_filename: str = CONSTRAINTS_FILE_NAME,
) -> dict[str, Any]:
    manifest: dict[str, Any] = {
        ...
        "sbom_policy": (
            "offline SPDX JSON SBOM plus package provenance and dependency inventory are "
            "generated; registry signing and attestation remain operator-owned"
        ),
        "dependency_integrity_policy": (
            "exact install constraints are recorded with an offline dependency integrity document; "
            "package artifact hash pinning remains enterprise wheelhouse-owned"
        ),
        "validation_commands": [
            ...
            f"python -m pip install -c {constraints_filename} <wheel>",
            "attackiq --version",
    ...
    if dependency_integrity_sha256 is not None:
        manifest["dependency_integrity_file"] = {
            "filename": dependency_integrity_filename,
...
def copy_constraints_file(source_dir: Path, output_dir: Path) -> tuple[Path, str]:
    source_path = source_dir / CONSTRAINTS_FILE_NAME
    if not source_path.is_file():
        raise RuntimeError(f"missing {CONSTRAINTS_FILE_NAME} in public source tag")
    target_path = output_dir / CONSTRAINTS_FILE_NAME
...
def build_enterprise_package(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="attackiq-cli-enterprise-source-") as tmpdir:
        ...
        constraints_path, constraints_sha256 = copy_constraints_file(source_dir, artifact_dir)
    ...
    sbom = package_sbom.build_package_sbom(
        ...
        wheel_sha256=wheel_sha256,
        generated_utc=build_created_utc,
    )
    sbom_path = package_sbom.write_package_sbom(artifact_dir, sbom)
    sbom_sha256 = sha256_file(sbom_path)
    dependency_integrity = package_dependency_integrity.build_dependency_integrity(
        constraints_path=constraints_path,
        constraints_sha256=constraints_sha256,
        generated_utc=build_created_utc,
    )
    dependency_integrity_path = package_dependency_integrity.write_dependency_integrity(
        artifact_dir,
        dependency_integrity,
    )
    dependency_integrity_sha256 = sha256_file(dependency_integrity_path)
    manifest = build_promotion_manifest(
        public_repo_url=public_repo_url,
        source_ref=source_ref,
        source_commit=source_commit,
        ...
        dependency_integrity_sha256=dependency_integrity_sha256,
        build_created_utc=build_created_utc,
    )
    ...
    checksum_path = write_sha256s(
        artifact_dir,
        {
            ...
            constraints_path.name: constraints_sha256,
            sbom_path.name: sbom_sha256,
            dependency_integrity_path.name: dependency_integrity_sha256,
    ...
    return {
        ...
        "sbom_path": sbom_path,
        "sbom_sha256": sbom_sha256,
        "dependency_integrity_path": dependency_integrity_path,
        "dependency_integrity_sha256": dependency_integrity_sha256,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Constraints: {summary['constraints_path']}")
    print(f"Constraints SHA256: {summary['constraints_sha256']}")
    print(f"SBOM: {summary['sbom_path']}")
    print(f"SBOM SHA256: {summary['sbom_sha256']}")
    print(f"Dependency integrity: {summary['dependency_integrity_path']}")
    print(f"Dependency integrity SHA256: {summary['dependency_integrity_sha256']}")
