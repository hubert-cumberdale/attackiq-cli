...
TRUST_ROOT_VERIFICATION_FIELDS = (
...
PUBLIC_REPOSITORY_EXCLUDED_VALUES = (
    "private keys",
    "signing tokens",
...
def _external_evidence_fields() -> dict[str, Any]:
    return {
        "external_record_policy": (
            "record real signature, attestation, and trust-root verification values only in the "
            "approved enterprise release system; repository-owned docs may reference these field "
            "names and public-safe pass/fail summaries"
        ),
        "signature_verification": list(SIGNATURE_VERIFICATION_FIELDS),
        "attestation_verification": list(ATTESTATION_VERIFICATION_FIELDS),
        "trust_root_verification": list(TRUST_ROOT_VERIFICATION_FIELDS),
        "public_repository_excluded_values": list(PUBLIC_REPOSITORY_EXCLUDED_VALUES),
    }
...
def build_signing_attestation_evidence(
    ...
    *,
    ...
    return {
        ...
        "external_evidence_fields": _external_evidence_fields(),
