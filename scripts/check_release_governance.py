...
PREPARED_CANDIDATE_RE = re.compile(
    r"Prepared release candidate:\s*`(?P<release>v\d+\.\d+\.\d+)`"
)
...
def extract_prepared_candidate(state_text: str) -> str | None:
    match = PREPARED_CANDIDATE_RE.search(state_text)
    if not match:
        return None
    return match.group("release")
...
def check_release_governance(root: Path = ROOT) -> list[str]:
    ...
    prepared_candidate = extract_prepared_candidate(state)
    if current_release is None:
        errors.append("docs/STATE.md must declare `Current production-ready release: `vX.Y.Z``.")
    elif current_release != expected_release and prepared_candidate != expected_release:
        errors.append(
            "docs/STATE.md must declare either the current production-ready release or prepared "
            "release candidate matching pyproject version: "
            f"current={current_release}, prepared={prepared_candidate}, "
            f"pyproject={expected_release}."
        )
