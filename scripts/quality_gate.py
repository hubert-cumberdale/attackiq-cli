...
def build_commands(*, include_mkdocs: bool = True) -> list[GateCommand]:
    ...
    commands = [
        ...
        GateCommand("secret scan", [python, "scripts/check_secret_scan.py"]),
        GateCommand(
        ...
        GateCommand(
            ...
            [python, "scripts/render_cli_contract.py", "--check"],
        ),
        GateCommand(
            "GA CLI inventory parity",
            [python, "scripts/check_ga_cli_inventory.py"],
        ),
        GateCommand(
            "architecture boundaries",
            [python, "scripts/deterministic_review.py", "--check-architecture"],
        ),
        GateCommand(
            "ruff",
            [
