...
REQUIRED_EXTERNAL_EVIDENCE_FIELDS = {
...
REQUIRED_PUBLIC_REPOSITORY_EXCLUSIONS = {
    "private keys",
    "signing tokens",
    "registry credentials",
    "private trust-root paths",
    "certificates",
    "operator names",
    "change-ticket identifiers",
    "internal audit-log URLs",
}
...
def expected_promotion_files(
    ...
    if isinstance(sbom_file, dict):
        ...
        artifact_type = sbom_file.get("type")
        if isinstance(filename, str) and isinstance(digest, str):
            entries[filename] = _file_entry(
                package_dir,
                filename,
                artifact_type=artifact_type if isinstance(artifact_type, str) else "spdx-json",
                sha256=digest,
            )
    ...
    if isinstance(dependency_integrity_file, dict):
        ...
        artifact_type = dependency_integrity_file.get("type")
        if isinstance(filename, str) and isinstance(digest, str):
            entries[filename] = _file_entry(
                package_dir,
                filename,
...
def _validate_external_evidence_fields(value: object) -> list[str]:
    errors: list[str] = []
    label = f"{SIGNING_EVIDENCE_NAME}: external_evidence_fields"
    fields = _as_dict(value, label=label, errors=errors)
    if fields is None:
        return errors

    policy = fields.get("external_record_policy")
    if not isinstance(policy, str) or "enterprise release system" not in policy:
        errors.append(f"{label}: external_record_policy must describe enterprise record storage")

    for group, required_fields in REQUIRED_EXTERNAL_EVIDENCE_FIELDS.items():
        field_values = _as_list(fields.get(group), label=f"{label}.{group}", errors=errors)
        if field_values is None:
            continue
        actual_fields = {item for item in field_values if isinstance(item, str)}
        for index, item in enumerate(field_values):
            if not isinstance(item, str):
                errors.append(f"{label}.{group}[{index}]: expected string")
        missing = sorted(required_fields - actual_fields)
        if missing:
            errors.append(f"{label}.{group} missing {', '.join(missing)}")

    excluded_values = _as_list(
        fields.get("public_repository_excluded_values"),
        label=f"{label}.public_repository_excluded_values",
        errors=errors,
    )
    if excluded_values is not None:
        actual_exclusions = {item for item in excluded_values if isinstance(item, str)}
        for index, item in enumerate(excluded_values):
            if not isinstance(item, str):
                errors.append(
                    f"{label}.public_repository_excluded_values[{index}]: expected string"
                )
        missing_exclusions = sorted(REQUIRED_PUBLIC_REPOSITORY_EXCLUSIONS - actual_exclusions)
        if missing_exclusions:
            errors.append(
                f"{label}.public_repository_excluded_values missing "
...
def validate_signing_evidence(
    ...
    *,
    ...
    errors.extend(_validate_external_evidence_fields(evidence.get("external_evidence_fields")))
