...
def validate_manifest_shape(manifest: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(source_ref, str) or not RELEASE_REF_RE.fullmatch(source_ref):
        errors.append(f"{MANIFEST_NAME}: source_ref must be a vX.Y.Z release tag")
    elif package_version != RELEASE_REF_RE.fullmatch(source_ref).group("version"):
...
def verify_artifacts(
    ...
    extra_checksums = set(checksums) - set(manifest_hashes)
...
def verify_enterprise_package(package_dir: Path) -> tuple[dict[str, Any] | None, list[str]]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary, errors = verify_enterprise_package(args.package_dir)
    if errors:
