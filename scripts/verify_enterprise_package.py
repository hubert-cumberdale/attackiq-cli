...
try:
    ...
    from scripts import package_dependency_integrity, package_provenance, package_sbom
except ModuleNotFoundError:  # pragma: no cover - direct script execution fallback.
    scripts_dir = Path(__file__).resolve().parent
    if str(scripts_dir) not in sys.path:
        sys.path.insert(0, str(scripts_dir))
    import check_public_safety as public_safety  # type: ignore[no-redef]
    import package_dependency_integrity  # type: ignore[no-redef]
    import package_provenance  # type: ignore[no-redef]
    import package_sbom  # type: ignore[no-redef]
...
SBOM_NAME = package_sbom.SBOM_NAME
DEPENDENCY_INTEGRITY_NAME = package_dependency_integrity.DEPENDENCY_INTEGRITY_NAME
...
def _sbom_file_entry(manifest: dict[str, Any]) -> dict[str, Any] | None:
    sbom_file = manifest.get("sbom_file")
    return sbom_file if isinstance(sbom_file, dict) else None


def _dependency_integrity_file_entry(manifest: dict[str, Any]) -> dict[str, Any] | None:
    dependency_integrity_file = manifest.get("dependency_integrity_file")
    return dependency_integrity_file if isinstance(dependency_integrity_file, dict) else None
...
def validate_manifest_shape(manifest: dict[str, Any]) -> list[str]:
    ...
    if sbom_file is not None:
        if not isinstance(sbom_file, dict):
            errors.append(f"{MANIFEST_NAME}: sbom_file must be an object")
        else:
            filename = sbom_file.get("filename")
            digest = sbom_file.get("sha256")
            if filename != SBOM_NAME:
                errors.append(f"{MANIFEST_NAME}: sbom_file.filename must be {SBOM_NAME}")
            if sbom_file.get("type") != "spdx-json":
                errors.append(f"{MANIFEST_NAME}: sbom_file.type must be spdx-json")
            if not isinstance(digest, str) or not SHA256_RE.fullmatch(digest):
                errors.append(
                    f"{MANIFEST_NAME}: sbom_file.sha256 must be a 64-character "
                    "lowercase hex digest"
                )

    dependency_integrity_file = manifest.get("dependency_integrity_file")
    if dependency_integrity_file is not None:
        if not isinstance(dependency_integrity_file, dict):
            errors.append(f"{MANIFEST_NAME}: dependency_integrity_file must be an object")
        else:
            filename = dependency_integrity_file.get("filename")
            digest = dependency_integrity_file.get("sha256")
            if filename != DEPENDENCY_INTEGRITY_NAME:
                errors.append(
                    f"{MANIFEST_NAME}: dependency_integrity_file.filename must be "
                    f"{DEPENDENCY_INTEGRITY_NAME}"
                )
            if dependency_integrity_file.get("type") != "dependency-integrity-json":
                errors.append(
                    f"{MANIFEST_NAME}: dependency_integrity_file.type must be "
                    "dependency-integrity-json"
                )
            if not isinstance(digest, str) or not SHA256_RE.fullmatch(digest):
                errors.append(
                    f"{MANIFEST_NAME}: dependency_integrity_file.sha256 must be a "
...
def verify_artifacts(
    ...
    sbom_file = _sbom_file_entry(manifest)
    if sbom_file is not None:
        filename = sbom_file.get("filename")
        expected_digest = sbom_file.get("sha256")
        if isinstance(filename, str) and isinstance(expected_digest, str):
            allowed_checksum_only.add(filename)
            if checksums.get(filename) != expected_digest:
                errors.append(f"{filename}: manifest SHA256 does not match {CHECKSUM_FILE_NAME}")
            sbom_path = package_dir / filename
            if not sbom_path.is_file():
                errors.append(f"{filename}: SBOM file is missing")
            else:
                actual_digest = sha256_file(sbom_path)
                if actual_digest != expected_digest:
                    errors.append(f"{filename}: file SHA256 does not match manifest")

    dependency_integrity_file = _dependency_integrity_file_entry(manifest)
    if dependency_integrity_file is not None:
        filename = dependency_integrity_file.get("filename")
        expected_digest = dependency_integrity_file.get("sha256")
        if isinstance(filename, str) and isinstance(expected_digest, str):
            allowed_checksum_only.add(filename)
            if checksums.get(filename) != expected_digest:
                errors.append(f"{filename}: manifest SHA256 does not match {CHECKSUM_FILE_NAME}")
            dependency_integrity_path = package_dir / filename
            if not dependency_integrity_path.is_file():
                errors.append(f"{filename}: dependency integrity file is missing")
            else:
                actual_digest = sha256_file(dependency_integrity_path)
                if actual_digest != expected_digest:
                    errors.append(f"{filename}: file SHA256 does not match manifest")
...
def verify_sbom(
    ...
    filename = sbom_file.get("filename")
    if not isinstance(filename, str) or not is_safe_artifact_name(filename):
        return [f"{MANIFEST_NAME}: sbom_file.filename must be a safe local filename"]

    errors: list[str] = []
    sbom_path = package_dir / filename
    expected_digest = checksums.get(filename)
    if expected_digest is None:
        errors.append(f"{filename}: SBOM file is missing from {CHECKSUM_FILE_NAME}")
    if not sbom_path.is_file():
        errors.append(f"{filename}: SBOM file is missing")
        return errors
    actual_digest = sha256_file(sbom_path)
    if expected_digest is not None and actual_digest != expected_digest:
        errors.append(f"{filename}: file SHA256 does not match {CHECKSUM_FILE_NAME}")

    sbom, sbom_errors = package_sbom.load_package_sbom(sbom_path)
    errors.extend(sbom_errors)
    if sbom is not None:
        errors.extend(
            package_sbom.validate_package_sbom(
                manifest=manifest,
                sbom=sbom,
...
def verify_dependency_integrity(
    ...
    dependency_integrity_file = _dependency_integrity_file_entry(manifest)
    if dependency_integrity_file is None:
        return []
    filename = dependency_integrity_file.get("filename")
    if not isinstance(filename, str) or not is_safe_artifact_name(filename):
        return [
            f"{MANIFEST_NAME}: dependency_integrity_file.filename must be a safe local filename"
        ]
    ...
    expected_digest = checksums.get(filename)
    if expected_digest is None:
        errors.append(f"{filename}: dependency integrity file is missing from {CHECKSUM_FILE_NAME}")
    if not dependency_integrity_path.is_file():
        errors.append(f"{filename}: dependency integrity file is missing")
        return errors
    actual_digest = sha256_file(dependency_integrity_path)
    ...
    integrity, integrity_errors = package_dependency_integrity.load_dependency_integrity(
        dependency_integrity_path
    )
    errors.extend(integrity_errors)
    if integrity is not None:
        errors.extend(
            package_dependency_integrity.validate_dependency_integrity(
                package_dir=package_dir,
                manifest=manifest,
                integrity=integrity,
                integrity_filename=filename,
            )
        )
    return errors
...
def verify_enterprise_package(
    ...
    *,
    ...
    errors.extend(verify_sbom(artifact_dir, manifest, checksums))
    errors.extend(verify_dependency_integrity(artifact_dir, manifest, checksums))
    ...
    return {
        ...
        "sbom_present": bool(_sbom_file_entry(manifest)),
        "dependency_integrity_present": bool(_dependency_integrity_file_entry(manifest)),
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"SBOM: {summary['sbom_present']}")
    print(f"Dependency integrity: {summary['dependency_integrity_present']}")
