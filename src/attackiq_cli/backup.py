...
def run_configuration_backup(
    ...
    _validate_requested_domains(domains, catalog)
...
def _validate_requested_domains(
    domains: tuple[str, ...],
    ...
    for domain in domains:
        ...
        if entry.classification in {"write-like", "unsupported"}:
            raise BackupError(
                f"Endpoint catalog domain '{domain}' is {entry.classification} and cannot be "
                "used by backup commands."
            )
        if entry.method != "GET":
            raise BackupError(
                f"Endpoint catalog domain '{domain}' uses method {entry.method}; only GET is "
                "allowed for backup commands."
            )
