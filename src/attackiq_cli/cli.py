...
def _write_apply_response_json(output: Path | None, payload: Any) -> None:
    if output is None:
        write_json(sys.stdout, payload)
        return
    output.parent.mkdir(parents=True, exist_ok=True)
...
def _run_mutation_command(
    ...
    *,
    ...
    try:
        context, effective_timeout = _prepare_apply_context(
            ctx, operation=operation, timeout=timeout, index=index
        )
        response = apply_request(context, effective_timeout)
    except httpx.HTTPError as exc:
        _print_http_error_and_exit(exc)

    _write_apply_response_json(output, response)
