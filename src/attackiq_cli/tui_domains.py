...
_READ_ONLY_PREVIEW_COMMAND_IDS_BY_TAB = {
    "tab_scenarios": frozenset({"preview:new-assessment"}),
    "tab_assessments": frozenset(
        {
    ...
    "tab_tests": frozenset({"preview:test-scenarios", "preview:test-status"}),
}

READ_ONLY_PREVIEW_COMMAND_IDS = frozenset(
    command_id
    for command_ids in _READ_ONLY_PREVIEW_COMMAND_IDS_BY_TAB.values()
...
def build_command_palette_entries() -> list[CommandPaletteEntry]:
    ...
    entries.extend(
        [
            CommandPaletteEntry(
                "preview:assessment-defaults",
                "Preview assessment default targets request",
                "Preview",
                shortcut="-",
                keywords=("assessment", "asset", "call plan", "defaults", "preview"),
            ),
            CommandPaletteEntry(
                "preview:assessment-from-template",
                "Preview assessment from template request",
                "Preview",
                shortcut="-",
                keywords=("assessment", "call plan", "preview", "template"),
            ),
            CommandPaletteEntry(
                "preview:assessment-run",
                "Preview assessment run request",
                "Preview",
                shortcut="-",
                keywords=("assessment", "call plan", "no request sent", "preview"),
            ),
            CommandPaletteEntry(
                "preview:new-assessment",
                "Preview new assessment request",
                "Preview",
                shortcut="-",
                keywords=("assessment", "call plan", "new assessment", "preview", "scenario"),
            ),
            CommandPaletteEntry(
                "preview:new-test",
                "Preview new test request",
                "Preview",
                shortcut="-",
                keywords=("assessment", "call plan", "new test", "no request sent", "preview"),
            ),
            CommandPaletteEntry(
                "preview:test-scenarios",
                "Preview test scenario assignment request",
                "Preview",
                shortcut="-",
                keywords=("call plan", "no request sent", "preview", "scenario", "test"),
            ),
            CommandPaletteEntry(
                "preview:test-status",
                "Preview test status request",
                "Preview",
                shortcut="-",
                keywords=("call plan", "no request sent", "preview", "status", "test"),
            ),
            CommandPaletteEntry(
                "refresh",
                "Refresh current tab",
...
def allowed_command_ids_for_tab(tab_id: str) -> set[str]:
    ...
    if domain is None:
        return allowed
    allowed.update(_READ_ONLY_PREVIEW_COMMAND_IDS_BY_TAB.get(tab_id, ()))
