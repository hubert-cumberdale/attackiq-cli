...
def _fixture_json(name: str) -> Any:
    fixture_path = Path("tests/fixtures/config_backup") / name
...
def test_observable_field_mappings_catalog_backup_redacts_declared_sensitive_fields(
    monkeypatch,
    tmp_path,
) -> None:
    client = ClientStub(
        {
            "endpoint_catalog_observable_field_mappings": [
                _fixture_json("observable_field_mappings_page.json")
            ],
        }
    )
    monkeypatch.setattr(backup, "build_client", lambda *_args, **_kwargs: ClientManager(client))
    options = backup.ConfigBackupOptions(
        output_dir=tmp_path / "backup",
        domains=("observable-field-mappings",),
        page_size=50,
        max_pages=1,
        company_id=None,
        endpoint_catalog=Path("docs/CONFIGURATION_BACKUP_ENDPOINT_CATALOG.example.json"),
        tenant_alias="tenant-a",
        command="attackiq backup configs --include observable-field-mappings",
        insecure=False,
        timeout=None,
    )

    manifest = backup.run_configuration_backup(_context(), options)

    assert client.calls == [
        (
            "endpoint_catalog_observable_field_mappings",
            {"page": 1, "page_size": 50, "include_inactive": False},
        )
    ]
    assert manifest["domains_requested"] == ["observable-field-mappings"]
    assert manifest["artifacts"] == [
        {
            ...
            "artifact": "observable-field-mappings.json",
            "record_count": 1,
            "redaction_status": "redacted",
            "redacted_field_count": 3,
            "redacted_paths": [
                "$.records[0].api_token",
                "$.records[0].client_secret",
                "$.records[0].configuration",
            ],
        }
    ]

    artifact_text = (tmp_path / "backup" / "observable-field-mappings.json").read_text(
        encoding="utf-8"
    )
    assert "raw-api-token" not in artifact_text
    assert "raw-client-secret" not in artifact_text
    assert "raw-authorization-value" not in artifact_text
    assert "hostname" in artifact_text
    assert "asset.name" in artifact_text


def test_observable_field_mappings_catalog_backup_rejects_unclassified_sensitive_field(
    monkeypatch,
    tmp_path,
) -> None:
    client = ClientStub(
        {
            "endpoint_catalog_observable_field_mappings": [
                _fixture_json("observable_field_mappings_unclassified_page.json")
            ],
        }
    )
    monkeypatch.setattr(backup, "build_client", lambda *_args, **_kwargs: ClientManager(client))
    ...
    with pytest.raises(backup.BackupError, match=r"\$\.records\[0\]\.password"):
        backup.run_configuration_backup(_context(), options)
