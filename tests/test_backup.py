...
def test_endpoint_catalog_rejects_write_like_backup_entries() -> None:
    ...
    entries = {entry.domain: entry for entry in backup.validate_endpoint_catalog(catalog)}
    with pytest.raises(backup.BackupError, match="write-like"):
        backup._validate_requested_domains(("tenant-sso",), entries)
...
def test_endpoint_catalog_example_validates() -> None:
    ...
    entries = backup.validate_endpoint_catalog(json.loads(example_path.read_text(encoding="utf-8")))
