...
def _ci_python_versions() -> list[str]:
    ...
    return cast(list[str], matrix["python-version"])
...
def test_ci_supports_exact_ga_python_matrix_with_quality_gate_parity() -> None:
    assert _ci_python_versions() == ["3.10", "3.11", "3.12", "3.13"]
    assert _quality_gate_script_commands() <= _ci_single_line_commands()
