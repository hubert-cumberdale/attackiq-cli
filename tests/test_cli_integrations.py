...
import attackiq_cli.cli_integrations as cli_integrations
...
def test_integrations_list_passes_schema_backed_filters(monkeypatch):
    ...
    monkeypatch.setattr(cli_integrations, "ensure_auth", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(
        cli_integrations.SpecIndex,
        "from_file",
        lambda *_args, **_kwargs: DummySpecIndex(),
    )
    monkeypatch.setattr(
        cli_integrations,
        "svc_list_integration_connectors",
        _svc_list_integration_connectors,
    )
    monkeypatch.setattr(cli_integrations, "write_json", _write_json)
    ...
    filters = cast(cli_integrations.IntegrationConnectorFilters, captured["filters"])
...
def test_integrations_list_csv_requires_output(monkeypatch):
    ...
    monkeypatch.setattr(cli_integrations, "load_config_or_exit", lambda: CliConfig())
    monkeypatch.setattr(
        cli_integrations,
        "resolve_base_url",
        lambda *_args, **_kwargs: "https://api.example.com",
