...
import attackiq_cli.cli_scenarios as cli_scenarios
...
def test_scenarios_list_uses_paginate_results(monkeypatch):
    ...
    monkeypatch.setattr(cli_scenarios, "load_config", lambda: CliConfig())
    monkeypatch.setattr(
        cli_scenarios, "resolve_base_url", lambda *_args, **_kwargs: "https://api.example.com"
    )
    monkeypatch.setattr(cli_scenarios, "warn_if_insecure_base_url", lambda *_args, **_kwargs: False)
    monkeypatch.setattr(cli_scenarios, "ensure_auth", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(
        cli_scenarios.SpecIndex, "from_file", lambda *_args, **_kwargs: DummySpecIndex()
    )
    monkeypatch.setattr(cli_scenarios, "svc_list_scenarios", _svc_list_scenarios)
    ...
    filters = cast(cli_scenarios.ScenarioFilters, captured["filters"])
...
def test_scenarios_show(monkeypatch):
    ...
    monkeypatch.setattr(cli_scenarios, "fetch_scenario_detail", _fetch_scenario_detail)
...
def test_scenarios_upload_dry_run_redacts_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_scenarios, "warn_if_insecure", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(cli_scenarios, "ensure_auth", lambda *_args, **_kwargs: [])
