...
def test_tags_list_uses_paginate_results(monkeypatch):
    ...
    monkeypatch.setattr(cli, "svc_list_tags", _svc_list_tags)
    ...
    filters = cast(cli.TagFilters, captured["filters"])
...
def test_tags_show_fetches_tag_detail(monkeypatch):
    ...
    monkeypatch.setattr(cli, "ensure_auth", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(cli.SpecIndex, "from_file", lambda *_args, **_kwargs: DummySpecIndex())
    monkeypatch.setattr(cli, "fetch_tag_detail", _fetch_tag_detail)
    monkeypatch.setattr(cli, "write_json", _write_json)
...
def test_tags_search_uses_search_query(monkeypatch):
    ...
    monkeypatch.setattr(cli, "ensure_auth", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(cli.SpecIndex, "from_file", lambda *_args, **_kwargs: DummySpecIndex())
    monkeypatch.setattr(cli, "svc_search_tags", _svc_search_tags)
    monkeypatch.setattr(cli.console, "print", lambda obj: printed.append(obj))
