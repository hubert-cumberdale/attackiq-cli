...
def test_templates_list_passes_schema_backed_filters(monkeypatch):
    ...
    monkeypatch.setattr(cli, "svc_list_templates", _svc_list_templates)
    ...
    filters = cast(cli.TemplateFilters, captured["filters"])
...
def test_templates_show_fetches_template_detail(monkeypatch):
    ...
    monkeypatch.setattr(cli, "ensure_auth", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(cli.SpecIndex, "from_file", lambda *_args, **_kwargs: DummySpecIndex())
    monkeypatch.setattr(cli, "fetch_template_detail", _fetch_template_detail)
    monkeypatch.setattr(cli, "write_json", _write_json)
