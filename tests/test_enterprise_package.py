...
def test_build_promotion_manifest_records_enterprise_boundary() -> None:
    ...
    assert manifest["sbom_policy"].startswith("SBOM/provenance generation is deferred")
...
def test_build_enterprise_package_writes_artifacts(
    ...
    assert wheel_path.name in checksum_path.read_text(encoding="utf-8")
    assert '"source_commit": "abc123"' in manifest_path.read_text(encoding="utf-8")
