...
def test_build_promotion_manifest_records_enterprise_boundary() -> None:
    manifest = enterprise_package.build_promotion_manifest(
        ...
        wheel_sha256="digest",
        constraints_sha256="constraints-digest",
    ...
    assert manifest["constraints_file"] == {
        "filename": enterprise_package.CONSTRAINTS_FILE_NAME,
        "sha256": "constraints-digest",
        "type": "install-constraints",
    }
    assert "sbom_file" not in manifest
    assert "dependency_integrity_file" not in manifest
    ...
    assert manifest["sbom_policy"].startswith("offline SPDX JSON SBOM")
    assert manifest["dependency_integrity_policy"].startswith("exact install constraints")
...
def test_build_enterprise_package_fails_on_wheel_safety_errors(
    ...
    def fake_clone(_repo_url: str, _source_ref: str, destination: Path) -> None:
        ...
        (destination / enterprise_package.CONSTRAINTS_FILE_NAME).write_text(
            "httpx==0.27.2\n",
            encoding="utf-8",
        )
...
def test_build_enterprise_package_writes_artifacts(
    ...
    sbom_path = summary["sbom_path"]
    dependency_integrity_path = summary["dependency_integrity_path"]
    constraints_path = summary["constraints_path"]
    assert wheel_path.exists()
    assert constraints_path.exists()
    assert sbom_path.exists()
    assert dependency_integrity_path.exists()
    ...
    assert constraints_path.name in checksum_text
    assert sbom_path.name in checksum_text
    assert dependency_integrity_path.name in checksum_text
    ...
    assert manifest["source_commit"] == "abc123"
    assert manifest["provenance_file"] == enterprise_package.PROVENANCE_NAME
    assert manifest["sbom_file"]["filename"] == enterprise_package.SBOM_NAME
    assert (
        manifest["dependency_integrity_file"]["filename"]
        == enterprise_package.DEPENDENCY_INTEGRITY_NAME
    )
    assert manifest["constraints_file"]["filename"] == enterprise_package.CONSTRAINTS_FILE_NAME
    assert sbom["spdxVersion"] == "SPDX-2.3"
    assert dependency_integrity["document_type"] == "attackiq-cli-dependency-integrity"
    assert dependency_integrity["constraints"]["pinned_dependency_count"] == 1
    assert provenance["source"]["source_commit"] == "abc123"
    assert provenance["install_constraints"]["filename"] == enterprise_package.CONSTRAINTS_FILE_NAME
