...
import attackiq_cli.services_asset_groups as services_asset_groups
...
def test_list_asset_groups_autopaginates_with_filters(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        services_asset_groups,
        "build_client",
        lambda *_args, **_kwargs: ClientManager(),
    )
    monkeypatch.setattr(services_asset_groups, "paginate_results", _paginate_results)
