...
from attackiq_cli import tui_provider as tui_provider_module
...
def _looks_like_write_command(command_id: str) -> bool:
    if command_id in READ_ONLY_PREVIEW_COMMAND_IDS:
        return False
    normalized = command_id.lower()
    write_tokens = ("apply", "create", "delete", "mutation", "preview", "run", "update", "write")
    return any(token in normalized for token in write_tokens)
...
def _mock_workspace(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        tui_provider_module,
        "_find_repo_root",
        lambda _start: Path("/repo/aiq-cli"),
    )
    monkeypatch.setattr(
        tui_provider_module.Path,
        "cwd",
...
async def test_tui_status_tab_summary_usage(monkeypatch):
    ...
    async with app.run_test() as pilot:
        ...
        assert "inspect local request previews in read-only mode" in text
        hint = str(app.query_one("#status_hint", Static).renderable)
        assert hint == "Read-only mode. Request previews never send requests."
...
async def test_tui_help_overlay_toggle_actions(monkeypatch):
    ...
    async with app.run_test() as pilot:
        ...
        assert "Read-only mode | Request previews never send requests" in str(overlay.renderable)
...
def test_tui_command_palette_exposes_only_read_only_command_ids() -> None:
    entries = build_command_palette_entries()
    entry_ids = {entry.command_id for entry in entries}

    assert entry_ids == READ_ONLY_COMMAND_IDS
    assert not any(_looks_like_write_command(entry.command_id) for entry in entries)
    for domain in TUI_DOMAIN_CONTROLLERS:
        allowed = allowed_command_ids_for_tab(domain.tab_id)
        assert allowed <= READ_ONLY_COMMAND_IDS
        assert not any(_looks_like_write_command(command_id) for command_id in allowed)
